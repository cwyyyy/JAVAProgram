...
//生成token,并用用户明文密码加密
    ...
    /**
     * 将当前用户信息以用户密码为密钥生成token并存入redis的方法
     */
    public String loginSign(CurrentUser currentUser, String password,LoginUser loginUser){
        //生成token
        String token = sign(currentUser, password);

        //同时把loginUser存入redis ---------  token ：loginUser
        redisUtil.set(token,loginUser,expireTime);//单位秒
    ...
    public static   CurrentUser getCurrentUser(String token) {
        if(StringUtils.isEmpty(token)){
            throw new BusinessException("发现token为空，");
        }
